...
- Elke Phase-8 CLI-run (Translation → Readability → Fidelity) MOET gevolgd worden door:
  - `sandbox/tools/phase8_output_contract_validator.sh <run_dir>`
- Technische gate (optioneel): gebruik `sandbox/tools/phase8_run_with_gate.sh <run_dir> <english_txt> <rough_nl_txt>`
- Output contract v2: hoofdtekst mag EN of NL zijn; consistentie is gewenst, maar taal is geen gate-criterium.
- Gate controleert scheiding van hoofdtekst en opmerkingen, en governance-termen.
- PASS-definitie: een Phase-8 run is PAS geldig als
  sandbox/phase8_runs/<run_id>/eval/output_contract_checks.txt
  overall_status: PASS bevat (validator exit code 0).
- FAIL (exit 1) = run telt niet als geldig resultaat; alleen diagnostiek.
- Phase-8 is afgerond (COMPLETED — REWORK ACCEPTED); PASS-run is niet vereist.
- Gate functioneert op FAIL-cases; format-repair is expliciet uitgesteld naar backlog.
- Een FAIL betekent:
  - de run telt niet als geldig Phase-8 resultaat
  - evaluatie mag plaatsvinden, maar export, hergebruik of vervolgpilots zijn geblokkeerd
    totdat een PASS-run bestaat
